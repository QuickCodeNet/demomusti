...
{
    public class SourceTypesControllerTests
    {
        private readonly Mock<ISourceTypeService> _serviceMock = new();
        private readonly Mock<ILogger<SourceTypesController>> _loggerMock = new();
        private readonly SourceTypesController _controller;
        public SourceTypesControllerTests()
        {
            _controller = new SourceTypesController(_serviceMock.Object, null, _loggerMock.Object);
        }
        ...
        {
            // Arrange
            var fakeList = TestDataGenerator.CreateFakes<SourceTypeDto>("tr");
            _serviceMock.Setup(s => s.ListAsync(It.IsAny<int?>(), It.IsAny<int?>())).ReturnsAsync(new Response<List<SourceTypeDto>> { Code = 0, Value = fakeList });
            // Act
        ...
        {
            // Arrange
            _serviceMock.Setup(s => s.ListAsync(It.IsAny<int?>(), It.IsAny<int?>())).ReturnsAsync(new Response<List<SourceTypeDto>> { Code = 1, Message = "Error" });
            // Act
            var result = await _controller.ListAsync(1, 10);
            // Assert
        ...
        public async Task GetItemAsync_Should_Return_Ok_When_Found()
        {
            ...
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>())).ReturnsAsync(new Response<SourceTypeDto> { Code = 0, Value = fakeDto });
            // Act
        ...
        public async Task GetItemAsync_Should_Return_NotFound_When_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<SourceTypeDto>("tr");
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>())).ReturnsAsync(new Response<SourceTypeDto> { Code = 404, Message = "Not found" });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.Id);
            // Assert
            Assert.IsType<NotFoundObjectResult>(result);
        ...
        public async Task GetItemAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<SourceTypeDto>("tr");
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>())).ReturnsAsync(new Response<SourceTypeDto> { Code = 1, Message = "Error" });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.Id);
            // Assert
        ...
        public async Task InsertAsync_Should_Return_Created_When_Success()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<SourceTypeDto>("tr");
            _serviceMock.Setup(s => s.InsertAsync(It.IsAny<SourceTypeDto>())).ReturnsAsync(new Response<SourceTypeDto> { Code = 0, Value = fakeDto });
            // Act
        ...
        public async Task InsertAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<SourceTypeDto>("tr");
            _serviceMock.Setup(s => s.InsertAsync(It.IsAny<SourceTypeDto>())).ReturnsAsync(new Response<SourceTypeDto> { Code = 1, Message = "Insert failed" });
            // Act
            var result = await _controller.InsertAsync(fakeDto);
            // Assert
            Assert.IsType<BadRequestObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_Ok_When_Success()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<SourceTypeDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<SourceTypeDto>())).ReturnsAsync(new Response<bool> { Code = 0, Value = true });
            // Act
        ...
        public async Task UpdateAsync_Should_Return_NotFound_When_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<SourceTypeDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<SourceTypeDto>())).ReturnsAsync(new Response<bool> { Code = 404, Message = "Not found" });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.Id, fakeDto);
            // Assert
            Assert.IsType<NotFoundObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<SourceTypeDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<SourceTypeDto>())).ReturnsAsync(new Response<bool> { Code = 1, Message = "Update failed" });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.Id, fakeDto);
            // Assert
            Assert.IsType<BadRequestObjectResult>(result);
