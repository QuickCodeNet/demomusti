...
using Moq;
using QuickCode.Demomusti.TrainingModule.Persistence.Repositories;
...
namespace QuickCode.Demomusti.TrainingModule.Persistence.Tests.Repositories
{
    public class TrainingCategoryAssignmentRepositoryTests : IDisposable
    {
        ...
        private Mock<ILogger<TrainingCategoryAssignmentRepository>> loggerMock = null;
        private readonly ITestOutputHelper _output;
        public TrainingCategoryAssignmentRepositoryTests(ITestOutputHelper output)
        {
            _output = output;
            CreateDatabase();
        }

        private TrainingCategoryAssignmentRepository CreateRepository()
        {
            var writeContext = new WriteDbContext(writeOptions);
            var readContext = new ReadDbContext(readOptions);
            return new TrainingCategoryAssignmentRepository(loggerMock.Object, writeContext, readContext);
        }
        ...
        private void CreateDatabase()
        {
            ...
            loggerMock = new Mock<ILogger<TrainingCategoryAssignmentRepository>>();
        }
        ...
        public async Task InsertAsync_Should_Insert_Item_Successfully()
        {
            ...
            var itemDto = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>("tr");
            // Act
        ...
        public async Task InsertAsync_Should_Generate_Unique_Ids()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItem1 = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>();
            var fakeItem2 = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>("tr");
            // Act
        ...
        public async Task GetByPkAsync_Should_Return_Item_When_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var itemDto = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>();
        ...
        public async Task GetByPkAsync_Should_Return_NotFound_When_Item_Not_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var notExistsItem = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>("tr");
            // Act
            var result = await repository.GetByPkAsync(notExistsItem.Id);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Equal("Not found in TrainingCategoryAssignment", result.Message);
        }
        ...
        [Fact]
        public async Task ListAsync_Should_Return_All_TrainingCategoryAssignment()
        {
            // Arrange
            CreateDatabase();
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<TrainingCategoryAssignmentDto>("tr");
        ...
        public async Task ListAsync_With_Pagination_Should_Return_Correct_Page()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<TrainingCategoryAssignmentDto>("en", 5);
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
        ...
        public async Task ListAsync_Should_Order_By_Id_Ascending()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<TrainingCategoryAssignmentDto>();
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
