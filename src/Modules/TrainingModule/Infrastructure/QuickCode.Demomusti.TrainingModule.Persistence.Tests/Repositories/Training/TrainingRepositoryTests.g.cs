...
namespace QuickCode.Demomusti.TrainingModule.Persistence.Tests.Repositories
{
    public class TrainingRepositoryTests : IDisposable
    {
        ...
        private Mock<ILogger<TrainingRepository>> loggerMock = null;
        private readonly ITestOutputHelper _output;
        public TrainingRepositoryTests(ITestOutputHelper output)
        {
            _output = output;
            CreateDatabase();
        }

        private TrainingRepository CreateRepository()
        {
            var writeContext = new WriteDbContext(writeOptions);
            var readContext = new ReadDbContext(readOptions);
            return new TrainingRepository(loggerMock.Object, writeContext, readContext);
        }
        ...
        private void CreateDatabase()
        {
            ...
            loggerMock = new Mock<ILogger<TrainingRepository>>();
        }
        ...
        public async Task InsertAsync_Should_Insert_Item_Successfully()
        {
            ...
            var itemDto = TestDataGenerator.CreateFake<TrainingDto>("tr");
            // Act
        ...
        public async Task InsertAsync_Should_Generate_Unique_Ids()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItem1 = TestDataGenerator.CreateFake<TrainingDto>();
            var fakeItem2 = TestDataGenerator.CreateFake<TrainingDto>("tr");
            // Act
        ...
        public async Task GetByPkAsync_Should_Return_Item_When_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var itemDto = TestDataGenerator.CreateFake<TrainingDto>();
        ...
        public async Task GetByPkAsync_Should_Return_NotFound_When_Item_Not_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var notExistsItem = TestDataGenerator.CreateFake<TrainingDto>("tr");
            // Act
            var result = await repository.GetByPkAsync(notExistsItem.Id);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Equal("Not found in Training", result.Message);
        }
        ...
        [Fact]
        public async Task ListAsync_Should_Return_All_Training()
        {
            // Arrange
            CreateDatabase();
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<TrainingDto>("tr");
        ...
        public async Task ListAsync_With_Pagination_Should_Return_Correct_Page()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<TrainingDto>("en", 5);
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
        ...
        public async Task ListAsync_Should_Order_By_Id_Ascending()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<TrainingDto>();
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
