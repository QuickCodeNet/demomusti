...
using QuickCode.Demomusti.TrainingModule.Api.Controllers;
...
namespace QuickCode.Demomusti.TrainingModule.Api.Tests
{
    public class TrainingCategoryAssignmentsControllerTests
    {
        private readonly Mock<ITrainingCategoryAssignmentService> _serviceMock = new();
        private readonly Mock<ILogger<TrainingCategoryAssignmentsController>> _loggerMock = new();
        private readonly TrainingCategoryAssignmentsController _controller;
        public TrainingCategoryAssignmentsControllerTests()
        {
            _controller = new TrainingCategoryAssignmentsController(_serviceMock.Object, null, _loggerMock.Object);
        }
        ...
        {
            // Arrange
            var fakeList = TestDataGenerator.CreateFakes<TrainingCategoryAssignmentDto>("tr");
            _serviceMock.Setup(s => s.ListAsync(It.IsAny<int?>(), It.IsAny<int?>())).ReturnsAsync(new Response<List<TrainingCategoryAssignmentDto>> { Code = 0, Value = fakeList });
            // Act
        ...
        {
            // Arrange
            _serviceMock.Setup(s => s.ListAsync(It.IsAny<int?>(), It.IsAny<int?>())).ReturnsAsync(new Response<List<TrainingCategoryAssignmentDto>> { Code = 1, Message = "Error" });
            // Act
            var result = await _controller.ListAsync(1, 10);
            // Assert
        ...
        public async Task GetItemAsync_Should_Return_Ok_When_Found()
        {
            ...
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>())).ReturnsAsync(new Response<TrainingCategoryAssignmentDto> { Code = 0, Value = fakeDto });
            // Act
        ...
        public async Task GetItemAsync_Should_Return_NotFound_When_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>("tr");
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>())).ReturnsAsync(new Response<TrainingCategoryAssignmentDto> { Code = 404, Message = "Not found" });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.Id);
            // Assert
            Assert.IsType<NotFoundObjectResult>(result);
        ...
        public async Task GetItemAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>("tr");
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>())).ReturnsAsync(new Response<TrainingCategoryAssignmentDto> { Code = 1, Message = "Error" });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.Id);
            // Assert
        ...
        public async Task InsertAsync_Should_Return_Created_When_Success()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>("tr");
            _serviceMock.Setup(s => s.InsertAsync(It.IsAny<TrainingCategoryAssignmentDto>())).ReturnsAsync(new Response<TrainingCategoryAssignmentDto> { Code = 0, Value = fakeDto });
            // Act
        ...
        public async Task InsertAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>("tr");
            _serviceMock.Setup(s => s.InsertAsync(It.IsAny<TrainingCategoryAssignmentDto>())).ReturnsAsync(new Response<TrainingCategoryAssignmentDto> { Code = 1, Message = "Insert failed" });
            // Act
            var result = await _controller.InsertAsync(fakeDto);
            // Assert
            Assert.IsType<BadRequestObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_Ok_When_Success()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<TrainingCategoryAssignmentDto>())).ReturnsAsync(new Response<bool> { Code = 0, Value = true });
            // Act
        ...
        public async Task UpdateAsync_Should_Return_NotFound_When_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<TrainingCategoryAssignmentDto>())).ReturnsAsync(new Response<bool> { Code = 404, Message = "Not found" });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.Id, fakeDto);
            // Assert
            Assert.IsType<NotFoundObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<TrainingCategoryAssignmentDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<TrainingCategoryAssignmentDto>())).ReturnsAsync(new Response<bool> { Code = 1, Message = "Update failed" });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.Id, fakeDto);
            // Assert
            Assert.IsType<BadRequestObjectResult>(result);
