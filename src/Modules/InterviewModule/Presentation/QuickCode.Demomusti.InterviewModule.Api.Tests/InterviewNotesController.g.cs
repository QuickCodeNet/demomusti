...
{
    public class InterviewNotesControllerTests
    {
        private readonly Mock<IInterviewNoteService> _serviceMock = new();
        private readonly Mock<ILogger<InterviewNotesController>> _loggerMock = new();
        private readonly InterviewNotesController _controller;
        public InterviewNotesControllerTests()
        {
            _controller = new InterviewNotesController(_serviceMock.Object, null, _loggerMock.Object);
        }
        ...
        {
            // Arrange
            var fakeList = TestDataGenerator.CreateFakes<InterviewNoteDto>("tr");
            _serviceMock.Setup(s => s.ListAsync(It.IsAny<int?>(), It.IsAny<int?>())).ReturnsAsync(new Response<List<InterviewNoteDto>> { Code = 0, Value = fakeList });
            // Act
        ...
        {
            // Arrange
            _serviceMock.Setup(s => s.ListAsync(It.IsAny<int?>(), It.IsAny<int?>())).ReturnsAsync(new Response<List<InterviewNoteDto>> { Code = 1, Message = "Error" });
            // Act
            var result = await _controller.ListAsync(1, 10);
            // Assert
        ...
        public async Task GetItemAsync_Should_Return_Ok_When_Found()
        {
            ...
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>())).ReturnsAsync(new Response<InterviewNoteDto> { Code = 0, Value = fakeDto });
            // Act
        ...
        public async Task GetItemAsync_Should_Return_NotFound_When_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<InterviewNoteDto>("tr");
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>())).ReturnsAsync(new Response<InterviewNoteDto> { Code = 404, Message = "Not found" });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.Id);
            // Assert
            Assert.IsType<NotFoundObjectResult>(result);
        ...
        public async Task GetItemAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<InterviewNoteDto>("tr");
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>())).ReturnsAsync(new Response<InterviewNoteDto> { Code = 1, Message = "Error" });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.Id);
            // Assert
        ...
        public async Task InsertAsync_Should_Return_Created_When_Success()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<InterviewNoteDto>("tr");
            _serviceMock.Setup(s => s.InsertAsync(It.IsAny<InterviewNoteDto>())).ReturnsAsync(new Response<InterviewNoteDto> { Code = 0, Value = fakeDto });
            // Act
        ...
        public async Task InsertAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<InterviewNoteDto>("tr");
            _serviceMock.Setup(s => s.InsertAsync(It.IsAny<InterviewNoteDto>())).ReturnsAsync(new Response<InterviewNoteDto> { Code = 1, Message = "Insert failed" });
            // Act
            var result = await _controller.InsertAsync(fakeDto);
            // Assert
            Assert.IsType<BadRequestObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_Ok_When_Success()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<InterviewNoteDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<InterviewNoteDto>())).ReturnsAsync(new Response<bool> { Code = 0, Value = true });
            // Act
        ...
        public async Task UpdateAsync_Should_Return_NotFound_When_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<InterviewNoteDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<InterviewNoteDto>())).ReturnsAsync(new Response<bool> { Code = 404, Message = "Not found" });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.Id, fakeDto);
            // Assert
            Assert.IsType<NotFoundObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<InterviewNoteDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<InterviewNoteDto>())).ReturnsAsync(new Response<bool> { Code = 1, Message = "Update failed" });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.Id, fakeDto);
            // Assert
            Assert.IsType<BadRequestObjectResult>(result);
