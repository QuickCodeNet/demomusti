...
{
    public class InterviewFeedbackAnswerRepositoryTests : IDisposable
    {
        ...
        private Mock<ILogger<InterviewFeedbackAnswerRepository>> loggerMock = null;
        private readonly ITestOutputHelper _output;
        public InterviewFeedbackAnswerRepositoryTests(ITestOutputHelper output)
        {
            _output = output;
            CreateDatabase();
        }

        private InterviewFeedbackAnswerRepository CreateRepository()
        {
            var writeContext = new WriteDbContext(writeOptions);
            var readContext = new ReadDbContext(readOptions);
            return new InterviewFeedbackAnswerRepository(loggerMock.Object, writeContext, readContext);
        }
        ...
        private void CreateDatabase()
        {
            ...
            loggerMock = new Mock<ILogger<InterviewFeedbackAnswerRepository>>();
        }
        ...
        public async Task InsertAsync_Should_Insert_Item_Successfully()
        {
            ...
            var itemDto = TestDataGenerator.CreateFake<InterviewFeedbackAnswerDto>("tr");
            // Act
        ...
        public async Task InsertAsync_Should_Generate_Unique_Ids()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItem1 = TestDataGenerator.CreateFake<InterviewFeedbackAnswerDto>();
            var fakeItem2 = TestDataGenerator.CreateFake<InterviewFeedbackAnswerDto>("tr");
            // Act
        ...
        public async Task GetByPkAsync_Should_Return_Item_When_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var itemDto = TestDataGenerator.CreateFake<InterviewFeedbackAnswerDto>();
        ...
        public async Task GetByPkAsync_Should_Return_NotFound_When_Item_Not_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var notExistsItem = TestDataGenerator.CreateFake<InterviewFeedbackAnswerDto>("tr");
            // Act
            var result = await repository.GetByPkAsync(notExistsItem.Id);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Equal("Not found in InterviewFeedbackAnswer", result.Message);
        }
        ...
        [Fact]
        public async Task ListAsync_Should_Return_All_InterviewFeedbackAnswer()
        {
            // Arrange
            CreateDatabase();
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<InterviewFeedbackAnswerDto>("tr");
        ...
        public async Task ListAsync_With_Pagination_Should_Return_Correct_Page()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<InterviewFeedbackAnswerDto>("en", 5);
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
        ...
        public async Task ListAsync_Should_Order_By_Id_Ascending()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<InterviewFeedbackAnswerDto>();
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
