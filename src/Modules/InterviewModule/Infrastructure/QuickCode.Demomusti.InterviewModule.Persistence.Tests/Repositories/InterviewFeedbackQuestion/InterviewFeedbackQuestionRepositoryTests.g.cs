...
using Moq;
using QuickCode.Demomusti.InterviewModule.Persistence.Repositories;
...
namespace QuickCode.Demomusti.InterviewModule.Persistence.Tests.Repositories
{
    public class InterviewFeedbackQuestionRepositoryTests : IDisposable
    {
        ...
        private Mock<ILogger<InterviewFeedbackQuestionRepository>> loggerMock = null;
        private readonly ITestOutputHelper _output;
        public InterviewFeedbackQuestionRepositoryTests(ITestOutputHelper output)
        {
            _output = output;
            CreateDatabase();
        }

        private InterviewFeedbackQuestionRepository CreateRepository()
        {
            var writeContext = new WriteDbContext(writeOptions);
            var readContext = new ReadDbContext(readOptions);
            return new InterviewFeedbackQuestionRepository(loggerMock.Object, writeContext, readContext);
        }
        ...
        private void CreateDatabase()
        {
            ...
            loggerMock = new Mock<ILogger<InterviewFeedbackQuestionRepository>>();
        }
        ...
        public async Task InsertAsync_Should_Insert_Item_Successfully()
        {
            ...
            var itemDto = TestDataGenerator.CreateFake<InterviewFeedbackQuestionDto>("tr");
            // Act
        ...
        public async Task InsertAsync_Should_Generate_Unique_Ids()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItem1 = TestDataGenerator.CreateFake<InterviewFeedbackQuestionDto>();
            var fakeItem2 = TestDataGenerator.CreateFake<InterviewFeedbackQuestionDto>("tr");
            // Act
        ...
        public async Task GetByPkAsync_Should_Return_Item_When_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var itemDto = TestDataGenerator.CreateFake<InterviewFeedbackQuestionDto>();
        ...
        public async Task GetByPkAsync_Should_Return_NotFound_When_Item_Not_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var notExistsItem = TestDataGenerator.CreateFake<InterviewFeedbackQuestionDto>("tr");
            // Act
            var result = await repository.GetByPkAsync(notExistsItem.Id);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Equal("Not found in InterviewFeedbackQuestion", result.Message);
        }
        ...
        [Fact]
        public async Task ListAsync_Should_Return_All_InterviewFeedbackQuestion()
        {
            // Arrange
            CreateDatabase();
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<InterviewFeedbackQuestionDto>("tr");
        ...
        public async Task ListAsync_With_Pagination_Should_Return_Correct_Page()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<InterviewFeedbackQuestionDto>("en", 5);
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
        ...
        public async Task ListAsync_Should_Order_By_Id_Ascending()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<InterviewFeedbackQuestionDto>();
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
